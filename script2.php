  ...
  if (isset($request[0]) && isset($request[1])) {
    ...
    if ($result->num_rows > 0) {
      ...
      http_response_code(404);
        ...
        $sql = "INSERT into tbl_users (username, firstname, lastname, age, email, zip) VALUES ('$username', '$fname','$lname', '$age','$email','$zip')";
        if ($mysqli->query($sql) === TRUE) {
          echo json_encode(array("username"=>$username,"lastname"=>$lname, "age"=>$age,"email"=>$email,"zip"=>$zip,"action"=>"completed"));
        }
